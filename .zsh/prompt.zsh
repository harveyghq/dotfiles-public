...
function RPR_HOST() {
    if [[ "${RPR_SHOW_HOST}" == "true" ]]; then
        echo "%{$fg[white]%}$(machine_name)%{$reset_color%}"
    fi
    return
...
# check if "http_proxy" and "https_proxy" are set, output "P" in red if so, else in green
function RPR_PROXY() {
    if [[ -z "${http_proxy}" || -z "${https_proxy}" ]]; then
        echo "%{$fg[red]%}P%{$reset_color%} "
    else
...
function RPR_INFO() {
    echo "$(RPR_PROXY)$(RPR_USER)$(RPR_AT)$(RPR_HOST)"
}
